...
/**
 * AndroidCamera is a modified version of KetaiCamera taken from Ketai Library
 * http://ket.ai for more info
 */
public class AndroidCamera extends PImage {
   ...
   */
  public AndroidCamera(PApplet pParent, int _width, int _height,
  ...
  private void determineObjectIntentions(Object o) {
    ...
    try {
      onPreviewEventMethodPImage = o.getClass().getMethod(
          "onCameraPreviewEvent", new Class[] { AndroidCamera.class });
    } catch (NoSuchMethodException e) {
      // no such method, or an error.. which is fine, just ignore
    ...
    try {
      onFaceDetectionEventMethod = o.getClass().getMethod(
          "onFaceDetectionEvent", new Class[] { AndroidCamera[].class });
    } catch (NoSuchMethodException e) {
      // no such method, or an error.. which is fine, just ignore
